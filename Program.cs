...
// Console.WriteLine("Characters with alias 'Snowmad King':");
// foreach (var character in characters.Where(c => c.Alias.Contains("Snowmad King")).Select(c => new { c.Name, c.Alias }))
// {
//     Console.WriteLine($"\t{character.Name} - Alias: [{string.Join(", ", character.Alias)}]");
// }

// // Do any character(s) that have an alias of Winter Kong (return type must be boolean)?
// bool charactersWithWinterKongAlias = characters.Any(c => c.Alias.Contains("Winter Kong"));
...
// Console.WriteLine("Human characters in Mario series:");
...
// int marioKoopaCharacters = characters.Count(c => c.Series.Contains("Mario") && c.Species == "Koopa");
// Console.WriteLine($"Koopa characters in Mario series: {marioKoopaCharacters}");

// List the character(s) in the Mario series that are Koopa species - return character name only.
// Console.WriteLine("Koopa characters in Mario series:");
// foreach (string? name in characters.Where(c => c.Series.Contains("Mario") && c.Species == "Koopa").Select(c => c.Name))
// {
//     Console.WriteLine($"\t{name}");
// }
